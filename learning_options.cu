...
__global__ void curand_setup_kernel(curandState *state, int filter_size)
{
	int blockId = blockIdx.x + blockIdx.y * gridDim.x;
...
__global__ void lateral_inhibition_depth_wise_mother_thread (Neuron *NeuronList, int network_size, int depth_ind_to_learn, int inhibit_time, CNN_struct *CNN_setttings, float *spike_flag, int total_depth_number){
	if (threadIdx.x==0){
    	for(int depth_iter=0;depth_iter<total_depth_number;depth_iter++){
    		//if (layer_iter==0) printf("Layer[%d] SpikeFlag: %d\n", layer_iter, spike_flag[layer_iter]);
            if(spike_flag[depth_iter]>0 && depth_iter>=depth_ind_to_learn){//use lateral inhibition
            	int SIZE_PER_SIDE = sqrt((float)network_size)+1;
...
//===========END of CNN special setting-up phase============
	...
	int training_time_each_img = input_int;
	int calculated_total_time = training_time_each_img*2000;
	#undef MAX_TIME
	#define MAX_TIME calculated_total_time
	printf("==Training Total Iter: %d==", MAX_TIME);
	int total_neuron_num = 0;
	...
	float min_frequency = 20;
	int training_set_number = 24;
...
//    curandGenerateUniform(gen_uniform, random_number_list_device, rand_numb_size);
    ...
    float *random_number_list_device;
    curandState *states;
    gpuErrchk(cudaMalloc((void **)&random_number_list_device, MAX_CONNECTION*sizeof(float)));
    gpuErrchk(cudaMalloc((void **)&states, MAX_CONNECTION* sizeof(curandState)));
	int SIZE_PER_SIDE_curand = sqrt((float)MAX_CONNECTION)+1;
	dim3 dimBlock_curand(4, 4);
	dim3 dimGrid_curand((SIZE_PER_SIDE/dimBlock.x+1), (SIZE_PER_SIDE/dimBlock.y+1));
	curand_setup_kernel<<<dimBlock_curand, dimGrid_curand>>>(states, MAX_CONNECTION);
...
//===========END of CNN special setting-up phase============
	...
	int training_time_each_img = input_int;
	int calculated_total_time = training_time_each_img*100000;
	#undef MAX_TIME
	#define MAX_TIME calculated_total_time
	printf("==Training Total Iter: %d==", MAX_TIME);
	int total_neuron_num = 0;
    ...
    float rand_list_size_to_total_connection_ratio = 1;
	int rand_numb_size = MAX_CONNECTION;
...
//    int reiter_run = 1;
	...
	while (time<max_time){
    	...
    	if(time==first_layer_time){
    		...
    		change_threshold<<<dimBlock_unit, dimGrid_unit>>>(Neuron_list_device, spiking_neuron_num, start_depth, end_depth, -70.2);
        	start_depth = network_config->layer[2].first_depth_id - 0.1;
        	end_depth = network_config->layer[2].last_depth_id + 0.1;
    		cout<<"Changing threshold, start: "<< start_depth<<" end: "<<end_depth<<endl;
    		change_threshold<<<dimBlock_unit, dimGrid_unit>>>(Neuron_list_device, spiking_neuron_num, start_depth, end_depth, -68.0);
    		...
    		cout<<"Changing threshold, start: "<< start_depth<<" end: "<<end_depth<<endl;
    		change_threshold<<<dimBlock_unit, dimGrid_unit>>>(Neuron_list_device, spiking_neuron_num, start_depth, end_depth, -69.0);
...
//    	    }
    	...
    	if(time<first_layer_time){
	    	...
	    	}else if(forced_lateral_inhibition_at_last_layer && CNN_total_layer_num==2){//if this is the last layer, use lateral_inhibition
	    		lateral_inhibition_mother_thread<<<dimBlock_unit, dimGrid_unit>>>(Neuron_list_device, spiking_neuron_num, 1, inhibition_time, d_network_config, spike_flag_device);
	    	}
	    	else{
	    		lateral_inhibition_mother_thread<<<dimBlock_unit, dimGrid_unit>>>(Neuron_list_device, spiking_neuron_num, 1, inhibition_time, d_network_config, spike_flag_device);
			...
			for(int layer_iter=0;layer_iter<CNN_total_layer_num;layer_iter++){
				...
				if (layer_iter==0) {//fault at convolution kernel and spiking cnn
					...
					synapse_drive_cnn_v2(Neuron_list_device, Input_neuronlist_device, network_config, d_network_config, d_filter_array, layer_iter, spiking_neuron_num, input_neuron_num, \
							syn_timer_max, connection_size, random_number_list_device, random_number_normal_device, states, -1.0, -1.0);//STDP
				}
			...
			for(int layer_iter=0;layer_iter<CNN_total_layer_num;layer_iter++){
				...
				if (layer_iter==0) {//fault at convolution kernel and spiking cnn
					...
					spiking_cnn_main(Neuron_list_device, Input_neuronlist_device, d_network_config, random_number_list_device, d_convolution_result, d_input_instance, \
							layer_iter, network_size, input_neuron_num, log_v, log_spike, log_total_spike, spike_flag_device, input_signal_width, 0.2*input_float, time, false);
					if (layer_iter!=(CNN_total_layer_num-1)) convolution_kernel(convolution_settings[layer_iter], layer_iter, h_input_instance, h_filter_array, h_convolution_result, probe);
				}else if(layer_iter==3){
					spiking_cnn_main(Neuron_list_device, Input_neuronlist_device, d_network_config, random_number_list_device, d_convolution_result, d_input_instance, \
							layer_iter, network_size, input_neuron_num, log_v, log_spike, log_total_spike, spike_flag_device, input_signal_width, 0.1*input_float, time, true);
					if (layer_iter!=(CNN_total_layer_num-1)) convolution_kernel(convolution_settings[layer_iter], layer_iter, h_input_instance, h_filter_array, h_convolution_result, probe);
					synapse_drive_cnn_v2(Neuron_list_device, Input_neuronlist_device, network_config, d_network_config, d_filter_array, layer_iter, spiking_neuron_num, input_neuron_num, syn_timer_max, connection_size, random_number_list_device, random_number_normal_device, states, -1.0, -1.0);//STDP
				}
	    	...
	    	}else if(apply_local_inhibition&& CNN_total_layer_num!=4){

	    	}else if(forced_lateral_inhibition_at_last_layer && CNN_total_layer_num==4){
	    		lateral_inhibition_mother_thread<<<dimBlock_unit, dimGrid_unit>>>(Neuron_list_device, spiking_neuron_num, 3, inhibition_time, d_network_config, spike_flag_device);
	    	}else{
	    		lateral_inhibition_mother_thread<<<dimBlock_unit, dimGrid_unit>>>(Neuron_list_device, spiking_neuron_num, 3, inhibition_time, d_network_config, spike_flag_device);
	    	}
			if(HOMEOSTASIS_ENABLE){
    				...
    				for(int layer_iter=0;layer_iter<CNN_total_layer_num;layer_iter++){
    		    		...
    		    		if (layer_iter==0) {
    						...
    						synapse_drive_cnn_v2(Neuron_list_device, Input_neuronlist_device, network_config, d_network_config, d_filter_array, layer_iter, spiking_neuron_num, input_neuron_num, syn_timer_max, \
    								connection_size, random_number_list_device, random_number_normal_device, states, -1.0, -1.0);//STDP
    		    		}
